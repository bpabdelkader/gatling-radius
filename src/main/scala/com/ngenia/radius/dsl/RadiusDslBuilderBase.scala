...
case class RadiusDslBuilderBase(
  requestName: String
){
...
case class RadiusDslBuilderPassword(
  ...
  def properties(properties: Map[String, Expression[Any]]) =
    RadiusDslBuilder(RadiusAttributes(requestName, username, None, properties), configuration)
}

case class RadiusDslBuilderProperties(
  configuration:    GatlingConfiguration,
  requestName:      String,
  username:         Expression[String],
  ...
  def properties(properties: Map[String, Expression[Any]]) =
    RadiusDslBuilder(RadiusAttributes(requestName, username, Some(password), properties), configuration)
}

case class RadiusDslBuilder(
  ...
  private def radiusActionBuilder(requestType: Type,
    radiusAttributes: RadiusAttributes,
    configuration: GatlingConfiguration
  ): ActionBuilder = RadiusActionBuilder
    .apply(
      requestType,
      radiusAttributes,
      configuration)
